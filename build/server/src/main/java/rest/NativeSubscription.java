...
import lists.CreatablesSubscriptionHelper;
...
public class NativeSubscription extends AbstractQueryService {
  ...
  @Autowired private ObjectFactory<CreatablesSubscriptionHelper> creatables;
  ...
  protected Flowable<JSONObject> executeOperation(Field field, JSONObject variables)
    ...
    switch (field.getName()) {
      ...
      case "onCreatableChangeEvent":
        {
          return subscription
              .onCreatableChangeEvent()
              .map((e) -> fromD3ESubscriptionEvent(e, field, "Creatable"));
        }
      case "onCreatableChangeEventById":
        {
          List<Long> ids = ctx.readLongColl("ids");
          return subscription
              .onCreatableChangeEvent()
