...
class Customers extends DBObject {
  ...
  void setItems(List<Customer> val) {
    ...
    if (!this.d3eChanges.contains(_ITEMS)) {
      List<Customer> _old = List.from(_items);
  ...
  void addToItems(Customer val, [int index = -1]) {
    List<Customer> _old = [];
  ...
  void removeFromItems(Customer val) {
    List<Customer> _old = [];
  ...
  void set(int field, Object value) {
    switch (field) {
      case _ITEMS:
        {
          this.setItems((value as List).cast<Customer>());
          break;
        }
    }
  }
  ...
  bool operator ==(Object other) {
    return identical(this, other) ||
        other is Customers && _items == other._items;
  }
