...
public class D3ESubscription implements FlowableOnSubscribe<DataStoreEvent> {
  ...
  public Flowable<D3ESubscriptionEvent<InventoryItem>> onInventoryItemChangeEvent() {
    return this.flowable
        .filter((e) -> e.getEntity() instanceof InventoryItem)
        .map(
            (e) -> {
              D3ESubscriptionEvent<InventoryItem> event = new D3ESubscriptionEvent<>();
              event.model = ((InventoryItem) e.getEntity());
              event.changeType = e.getType();
              return event;
            });
  }
  ...
  public Flowable<D3ESubscriptionEvent<Order>> onOrderChangeEvent() {
    return this.flowable
        .filter((e) -> e.getEntity() instanceof Order)
        .map(
            (e) -> {
              D3ESubscriptionEvent<Order> event = new D3ESubscriptionEvent<>();
              event.model = ((Order) e.getEntity());
              event.changeType = e.getType();
              return event;
            });
  }
