...
public class Inventory extends DBObject {
  public static final int _ITEMS = 0;
  private long id;
  private List<InventoryItem> items = ListExt.List();
  ...
  public void addToItems(InventoryItem val, long index) {
    collFieldChanged(_ITEMS, this.items);
    if (index == -1) {
      this.items.add(val);
    } else {
  ...
  public void removeFromItems(InventoryItem val) {
    collFieldChanged(_ITEMS, this.items);
  ...
  @Override
  public int _typeIdx() {
    return SchemaConstants.Inventory;
  }
