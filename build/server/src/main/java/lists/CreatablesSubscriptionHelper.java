...
public class CreatablesSubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = creatablesImpl.getAsJson(field, event.nativeData);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = creatablesImpl.getNativeResult();
  ...
  private void addSubscriptions() {
    ...
    */
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<Creatable>>) subscription.onCreatableChangeEvent())
            .subscribe((e) -> applyCreatable(e));
    disposables.add(baseSubscribe);
  }

  public void applyCreatable(D3ESubscriptionEvent<Creatable> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    Creatable model = e.model;
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, Creatable model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createCreatableData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, Creatable model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, Creatable model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private List<NativeObj> createCreatableData(Creatable creatable) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(creatable.getId());
    data.add(row);
    return data;
  }
