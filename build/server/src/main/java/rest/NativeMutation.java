...
import helpers.CreatableEntityHelper;
...
public class NativeMutation extends AbstractQueryService {
  ...
  private Object executeOperation(String query, Field field, JSONObject variables)
    ...
    switch (field.getName()) {
      case "createCreatable":
        {
          return createSuccessResult(createCreatable(ctx), field, "Creatable");
        }
      case "updateCreatable":
        {
          return createSuccessResult(updateCreatable(ctx), field, "Creatable");
        }
      case "deleteCreatable":
        {
          deleteCreatable(ctx);
          return createSuccessResult(null, field, "Creatable");
        }
    }
  ...
  private Creatable createCreatable(GraphQLInputContext ctx) throws Exception {
    ...
    Creatable newCreatable = ctx.readChild("input", "Creatable");
    this.mutator.save(newCreatable, false);
    return newCreatable;
  }

  private Creatable updateCreatable(GraphQLInputContext ctx) throws Exception {
    User currentUser = CurrentUser.get();
    if (!(currentUser instanceof AnonymousUser)) {
      throw new ValidationFailedException(
          MutateResultStatus.AuthFail,
    ...
    currentCreatable.recordOld(CloneContext.forCloneable(currentCreatable, false));
    Creatable newCreatable = ctx.readChild("input", "Creatable");
    this.mutator.update(newCreatable, false);
    return newCreatable;
  }

  private void deleteCreatable(GraphQLInputContext ctx) throws Exception {
    User currentUser = CurrentUser.get();
    if (!(currentUser instanceof AnonymousUser)) {
      throw new ValidationFailedException(
          MutateResultStatus.AuthFail,
    ...
    this.mutator.delete(currentCreatable, false);
  }
