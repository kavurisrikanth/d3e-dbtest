...
public class InventorySubscriptionHelper implements FlowableOnSubscribe<DataQueryDataChange> {
  ...
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = inventoryImpl.getAsJson(field, event.nativeData);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = inventoryImpl.getNativeResult();
  ...
  private void addSubscriptions() {
    ...
    */
    Disposable baseSubscribe =
        ((Flowable<D3ESubscriptionEvent<InventoryItem>>) subscription.onInventoryItemChangeEvent())
            .subscribe((e) -> applyInventoryItem(e));
    disposables.add(baseSubscribe);
  }

  public void applyInventoryItem(D3ESubscriptionEvent<InventoryItem> e) {
    List<DataQueryDataChange> changes = ListExt.List();
    InventoryItem model = e.model;
  ...
  private void createInsertChange(List<DataQueryDataChange> changes, InventoryItem model) {
    DataQueryDataChange change = new DataQueryDataChange();
    change.nativeData = createInventoryItemData(model);
  ...
  private void createUpdateChange(List<DataQueryDataChange> changes, InventoryItem model) {
  ...
  private void createDeleteChange(List<DataQueryDataChange> changes, InventoryItem model) {
    Row row = output.get(model.getId());
    if (row == null) {
      return;
    }
  ...
  private List<NativeObj> createInventoryItemData(InventoryItem inventoryItem) {
    List<NativeObj> data = ListExt.List();
    NativeObj row = new NativeObj(inventoryItem.getId());
    data.add(row);
    return data;
  }
